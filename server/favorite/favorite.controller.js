...
exports.likeUnlike = async (req, res, next) => {
  try {
    ...
    if (req.body.postId) {
      ...
      const compareDate =
        user.expiration_date <= postDate ? user.expiration_date : postDate;
      ...
      if (likeExist.length === 0) {
        ...
        favorite.expiration_date = compareDate;
    ...
    if (req.body.videoId) {
      ...
      const compareDate =
        user.expiration_date <= videoDate ? user.expiration_date : videoDate;

      console.log("user.expiration_date", user.expiration_date);
