...
exports.loginSignup = async (req, res) => {
  try {
    ...
    newUser.expiration_date = new Date(Date.now() + 3600 * 1000);
    ...
    income.expiration_date = user.expiration_date;
  ...
  try {
    ...
    if (req.body.imageType == 1) {
      user.image = req.files.image
        ? config.SERVER_PATH + req.files.image[0].path
        : null;
      user.imageType = req.body.imageType;
    }
    ...
    if (req.body.linkType == 1) {
      user.link = req.files.link
        ? config.SERVER_PATH + req.files.link[0].path
        : null;
      user.linkType = req.body.linkType;
    }
...
exports.updateFakeUser = async (req, res) => {
  try {
    ...
    if (req.body.imageType == 0) {
      ...
      user.imageType = req.body.imageType ? req.body.imageType : user.imageType;
    }
    ...
    if (req.body.linkType == 0) {
      ...
      user.link = req.body.link ? req.body.link : user.link;
      user.linkType = req.body.linkType ? req.body.linkType : user.linkType;
    }
    if (req.body.linkType == 1) {
      if (req.files.link) {
        var link_ = user.link?.split("storage");
...
exports.referralCode = async (req, res) => {
  try {
    ...
    if (!user.isReferral) {
      ...
      const compareDate =
        referralCodeUser.expiration_date <= user.expiration_date
          ? referralCodeUser.expiration_date
    ...
    return res.status(200).json({
      ...
      user:
        shuffleUser.length > 0
          ...
          : { ...fakeUser[0]._doc, userId: fakeUser[0]._id },
    });
  } catch (error) {
    console.log(error);
    return res
...
exports.addLessRcoinDiamond = async (req, res) => {
  try {
    ...
    wallet.expiration_date = user.expiration_date;
