...
exports.uploadVideo = async (req, res) => {
  try {
    ...
    video.expiration_date = user.expiration_date;
    await video.save();
...
exports.uploadFakeVideo = async (req, res) => {
  console.log("reqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq", req.body);
  console.log("reqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq", req.files);
  ...
  try {
    ...
    if (req.body.fakeVideoType == 1) {
      video.video = req.files.video ? config.SERVER_PATH + req.files.video[0].path : null;
      video.fakeVideoType = req.body.fakeVideoType ? req.body.fakeVideoType : null;
    }
...
exports.updateFakeVideo = async (req, res) => {
  ...
  try {
    ...
    if (req.body.fakeVideoType == 0) {
      ...
      video.video = req.body.video ? req.body.video : video.video;
      video.fakeVideoType = req.body.fakeVideoType ? req.body.fakeVideoType : video.fakeVideoType;
    }
    ...
    video.userId = req.body.userId ? req.body.userId : video.userId;
    video.hashtag = hashtagList;
    ...
    video.thumbnail = req.files.thumbnail ? config.SERVER_PATH + req.files.thumbnail[0].path : video.thumbnail;
    video.screenshot = req.files.screenshot ? config.SERVER_PATH + req.files.screenshot[0].path : video.screenshot;
  ...
  try {
    ...
    if (req.query.isFake === "false") {
      ...
      return res.status(200).json({
        status: true,
        message: "Success!!",
        video: video[0].video.length > 0 ? video[0].video : fakeVideo[0].video,
      });
    }
  } catch (error) {
    console.log(error);
